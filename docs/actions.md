...
device. `inspect` is `you`'s for the same reason: reading a card is looking, not
acting.

Naming a value and pinning it to the board are **not actions here at all**. They
are mywant's own operations, done in mywant — `mywant thing add`, or the Thing
page, or a want that names one in passing. The fortress's doors read mywant and
open for a value however it got there. An RPG verb wrapping the same call would
have taught a player something that works nowhere outside this game, which is
the opposite of what the game is for. An action asked by the wrong actor is refused with
