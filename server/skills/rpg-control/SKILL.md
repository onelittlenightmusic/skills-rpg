---
name: rpg-control
description: rpg-server に対してゲームアクション (open / activate / move / pickup / advance / return / inspect など) を実行する。既定は `chap`（`you` から拒否されたアクションの代行）。`actor` を `you` にすれば移動・ステージ移動・調査も呼べる。
compatibility:
  python: ">=3.9"
metadata:
...
アクターは2人で、できることが分かれている（chap は扉とデバイス、you は移動・取得・ステージ移動・調査）。誰が何を実行できるか、前提条件は何かは [docs/actions.md](https://github.com/onelittlenightmusic/skills-rpg/blob/main/docs/actions.md) を参照。
...
| `action` | string | ✓ | — | ゲームアクション名（observe / move / pickup / open / advance / return / inspect など） |
...
### 城塞都市（world: fortress）— 調べる

盤面の扉やアイテムが何を待っているかを読む。`you` の動作で、chap は代行できない
——世界への介入ではなく、見ることだから。

```bash
# 扉が何を待っているか（waiting_for / subtype / satisfied が返る）
...
**値に名前をつける・盤面に留めるのは、このスキルの仕事ではない。** それは mywant
そのものの操作で、`mywant thing add people Lira` や Thing ページで行う。城塞の扉は
mywant を読むので、どの経路で作られた値でも開く。
